...
import { getDatasetStatus } from '../services/videoService';
          ...
          <div className="bg-[#0b1426]/60 border border-navy-border/60 p-4 rounded-xl flex items-center gap-4">
            ...
            <div>
              ...
              <span className="block text-xl font-bold text-slate-200 font-mono mt-0.5">
                {stats.annotated_frames} <span className="text-xs text-slate-500 font-normal">/ {stats.total_frames}</span>
              </span>
            </div>
          </div>
          <div className="bg-[#0b1426]/60 border border-navy-border/60 p-4 rounded-xl flex items-center gap-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {stats.videos.map((vid) => {
                ...
                return (
                  <div
                    key={vid.video_id}
                    className="p-5 bg-navy-panel hover:bg-navy-panel/85 border border-navy-border hover:border-sky-500/30 rounded-xl transition-all duration-200 shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group"
                  >
                    <div className="space-y-2 min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-6 text-xs text-slate-500">
                        <span>Total frames: <strong className="text-slate-400 font-semibold">{vid.total_frames}</strong></span>
                    ...
                    <button
                      ...
                      className="w-full md:w-auto px-4 py-2 bg-navy-card hover:bg-sky-600 border border-navy-border hover:border-transparent text-slate-300 hover:text-white text-xs font-semibold rounded-lg transition-all flex items-center justify-center gap-1 group-hover:shadow-[0_0_8px_rgba(3,105,161,0.2)]"
                    >
